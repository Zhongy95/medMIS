...
public class CacheAspect {
     ...
     * 声明部门的切面表达式
     */
    private static final String POINTCUT_DEPT_ADD="execution(* com.group7.sys.service.impl.DeptServiceImpl.save(..))";
    private static final String POINTCUT_DEPT_UPDATE="execution(* com.group7.sys.service.impl.DeptServiceImpl.updateById(..))";
    private static final String POINTCUT_DEPT_GET="execution(* com.group7.sys.service.impl.DeptServiceImpl.getById(..))";
    private static final String POINTCUT_DEPT_DELETE="execution(* com.group7.sys.service.impl.DeptServiceImpl.removeById(..))";
    ...
    public Object cacheDeptAdd(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (res){
            CACHE_CONTAINER.put(CACHE_DEPT_PROFIX + object.getDeptId(),object);
        }
        return res;
    }
...
//            CACHE_CONTAINER.put(CACHE_DEPT_PROFIX+res2.getDeptId(),res2);
//            return res2;
//        }
//    }
    ...
    public Object cacheDeptUpdate(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (isSuccess){
            Dept dept =(Dept) CACHE_CONTAINER.get(CACHE_DEPT_PROFIX + deptVo.getDeptId());
            if (null==dept){
                dept=new Dept();
            }
            BeanUtils.copyProperties(deptVo,dept);
            log.info("部门对象缓存已更新"+CACHE_DEPT_PROFIX + deptVo.getDeptId());
            CACHE_CONTAINER.put(CACHE_DEPT_PROFIX+dept.getDeptId(),dept);
        }
        return isSuccess;
    }
     ...
     * 声明用户的切面表达式
     */
    private static final String POINTCUT_USER_UPDATE="execution(* com.group7.sys.service.impl.UserServiceImpl.updateById(..))";
    private static final String POINTCUT_USER_ADD="execution(* com.group7.sys.service.impl.UserServiceImpl.updateById(..))";
    private static final String POINTCUT_USER_GET="execution(* com.group7.sys.service.impl.UserServiceImpl.getById(..))";
    private static final String POINTCUT_USER_DELETE="execution(* com.group7.sys.service.impl.UserServiceImpl.removeById(..))";
    ...
    public Object cacheUserAdd(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Boolean res = (Boolean) joinPoint.proceed();
        if (res){
            CACHE_CONTAINER.put(CACHE_USER_PROFIX + object.getUserId(),object);
        }
        return res;
    }
    ...
    public Object cacheUserGet(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (res1!=null){
            ...
            CACHE_CONTAINER.put(CACHE_USER_PROFIX+res2.getUserId(),res2);
            return res2;
        }
    }
    ...
    public Object cacheUserUpdate(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        Boolean isSuccess = (Boolean) joinPoint.proceed();
        if (isSuccess){
            User user =(User) CACHE_CONTAINER.get(CACHE_USER_PROFIX + userVo.getUserId());
            if (null==user){
                user=new User();
            }
            BeanUtils.copyProperties(userVo,user);
            log.info("用户对象缓存已更新"+CACHE_USER_PROFIX + userVo.getUserId());
            CACHE_CONTAINER.put(CACHE_USER_PROFIX+user.getUserId(),user);
        }
        return isSuccess;
    }
